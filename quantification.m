...
for i=1:8
    factor=2^i;
    particion=linspace(min(X),max(X),factor+1);%aqui esta el error desde 2 hasta el size-1
    particiones=[particion(2:length(particion)-1)];
    vcuantizados=(particion(2:length(particion))+particion(1:length(particion)-1))./2;
    
   [index,Q_X]= quantiz(X_O,particiones,vcuantizados);
